...
public class EmployeeController {
	 ...
	 * updateEmployee(@RequestBody EmployeeInfoBean bean) {
	 * 
	 ...
	 * 
	 * for (EmployeeEducationalInfoBean educationInfoBean:
	 * bean.getLstEducationalInfo()) {
	 * educationInfoBean.getPkBean().setInfoBean(bean); }
	 * 
	 * for (EmployeeExperienceInfoBean experienceInfoBean :
	 * bean.getLstExperienceInfo()) {
	 * experienceInfoBean.getExpPKBean().setInfoBean(bean); }
	 * 
	 * EmployeeInfoBean manager = bean.getMngId();
	 * 
	 * if(manager!=null) { manager = repository.findById(manager.getId()).get();
	 ...
	 * );
	 * 
	 * repository.save(bean); response.setStatusCode(201);
	 * response.setMessage("success");
	 * response.setDescription("employee data updated successfully"); } else {
	 * response.setStatusCode(401); response.setMessage("fail");
	 * response.setDescription("employee data update failure"); }
	 * 
	 * return response; }// end of updateEmployee
	 * 
	 * @GetMapping(path = "/get", produces = MediaType.APPLICATION_JSON_VALUE)
	 * public EmployeeResponse getEmployee(int id) { EmployeeInfoBean bean =
	 * repository.findById(id).get(); EmployeeResponse response = new
	 * EmployeeResponse(); if (bean != null) { response.setStatusCode(201);
	 * response.setMessage("success");
	 * response.setDescription("employee data found successfully");
	 * response.setLstInfoBean(Arrays.asList(bean)); } else {
	 * response.setStatusCode(401); response.setMessage("fail");
	 ...
	 * public EmployeeResponse deleteEmployee(int id) { EmployeeResponse response =
	 * new EmployeeResponse(); if (repository.existsById(id)) {
	 * repository.deleteById(id); response.setStatusCode(201);
	 * response.setMessage("success");
	 * response.setDescription("employee data deleted successfully"); } else {
	 * response.setStatusCode(401); response.setMessage("fail");
	 * response.setDescription("employee data deleted failed"); }
	 * 
	 * return response; }// end of deleteEmployee
	 * 
	 * @GetMapping(value = "/getOtherInfo" , produces =
	 * MediaType.APPLICATION_JSON_VALUE) public EmployeeOtherInfoBean otherInfo(int
	 * id) { EmployeeInfoBean infoBean = new EmployeeInfoBean(); infoBean.setId(id);
